...
export default function CalendarView() {
  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-100/40 dark:bg-slate-950 relative">
      {/* Alert banner */}
      {showAlert && (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 p-8 flex flex-col overflow-y-auto min-w-0">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-1">
                  <button onClick={prevMonth} className="p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-850 transition-colors">
                    <ChevronLeft className="w-5 h-5 text-slate-500 dark:text-slate-400" />
                  </button>
                  <button onClick={nextMonth} className="p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-850 transition-colors">
                    <ChevronRight className="w-5 h-5 text-slate-500 dark:text-slate-400" />
                  </button>
                </div>
              </div>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mt-1">Revisa tus metas y tareas pendientes para hoy.</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-bold text-sm shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800 dark:text-slate-300"
              >
          ...
          <div className="flex-1 bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col min-h-[500px] dark:bg-slate-900 dark:border-slate-800">
            {/* Weekdays */}
            <div className="grid grid-cols-7 gap-2 mb-2">
              {daysOfWeek.map((day) => (
                <div key={day} className="text-center font-bold text-xs text-slate-400 dark:text-slate-500 py-2">
                  {day}
                </div>
              ))}
            </div>
              ...
              {daysGrid.map((day, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`min-h-[90px] border border-slate-100 dark:border-slate-800/60 rounded-xl p-2 flex flex-col justify-between transition-all ${
                      day
                        ? 'bg-slate-50/30 hover:bg-indigo-50/30 hover:border-indigo-200 hover:shadow-sm cursor-pointer dark:bg-slate-850/40 dark:hover:bg-indigo-950/20 dark:hover:border-indigo-800/60'
                        : 'bg-transparent border-none'
                    } ${isToday ? 'ring-2 ring-indigo-500/20 bg-indigo-50/20 hover:bg-indigo-50/40 dark:ring-indigo-500/40 dark:bg-indigo-950/40 dark:hover:bg-indigo-950/60' : ''}`}
                  >
                    {day && (
                      <>
                        <div className="flex justify-between items-center">
                          <span className={`text-xs font-bold ${isToday ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-800 dark:text-slate-350'}`}>
                            {day}
                          </span>
                          {dayTasks.length > 0 && (
        ...
        {/* Sidebar Tasks Panel */}
        <aside className="w-80 border-l border-slate-200 bg-white p-6 flex flex-col justify-between overflow-y-auto dark:border-slate-800 dark:bg-slate-900">
          <div className="flex-1 flex flex-col gap-6">
            <div>
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-extrabold text-slate-800 dark:text-slate-100 text-lg">Próximas Tareas</h3>
                <button
                  onClick={() => setIsModalOpen(true)}
              ...
              <div className="space-y-4">
                {tasks.slice(0, 4).map((task) => {
                  ...
                  let darkStatusClass = '';
                  if (status.label === 'VENCIDO') darkStatusClass = 'dark:bg-rose-950/40 dark:text-rose-400';
                  else if (status.label === 'POR VENCER') darkStatusClass = 'dark:bg-amber-950/40 dark:text-amber-400';
                  else if (status.label === 'EN TIEMPO') darkStatusClass = 'dark:bg-emerald-950/40 dark:text-emerald-400';
                  else darkStatusClass = 'dark:bg-blue-950/40 dark:text-blue-400';

                  return (
                    <div key={task.id} className="p-4 border border-slate-200/80 rounded-2xl hover:border-slate-300 hover:shadow-sm transition-all group dark:border-slate-800 dark:hover:border-slate-700">
                      <div className="flex items-center justify-between mb-2">
                        <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full ${status.colorClass} ${darkStatusClass}`}>
                          {status.label}
                        </span>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-medium text-slate-400 dark:text-slate-500">{status.timeLabel}</span>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              openConfirmModal(task.id);
                            }}
                            className="text-slate-300 hover:text-rose-600 dark:text-slate-600 dark:hover:text-rose-400 transition-colors duration-200"
                      ...
                      <h4 className="font-bold text-slate-800 text-sm mb-1 leading-snug group-hover:text-indigo-600 dark:text-slate-200 dark:group-hover:text-indigo-400 transition-colors">
                        {task.title}
                      </h4>
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-1.5">
                          <span
                            ...
                            style={{ backgroundColor: task.category?.color || '#a855f7' }}
                          ></span>
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold">{task.category?.name || 'Gral'}</span>
                        </div>
                        <button className="text-slate-400 dark:text-slate-500 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 text-xs font-semibold flex items-center gap-0.5">
                          →
                        </button>
                      </div>
                    </div>
            ...
            <div className="pt-6 border-t border-slate-200/80 dark:border-slate-800 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-indigo-600 dark:text-indigo-450 font-bold bg-indigo-50 dark:bg-indigo-950/60 px-2 py-0.5 rounded-full">
                  {ideas.length}
                </span>
              </div>
              ...
              <form onSubmit={handleAddIdea} className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 font-medium text-slate-800 dark:bg-slate-850 dark:border-slate-700 dark:text-slate-100 dark:focus:ring-indigo-500/40 dark:focus:border-indigo-500"
                />
                <button
                  type="submit"
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {ideas.length === 0 ? (
                  <p className="text-[11px] text-slate-400 dark:text-slate-500 font-semibold italic text-center py-2">
                  ...
                  ideas.map((idea) => (
                    <div
                      key={idea.id}
                      className="flex items-start justify-between gap-2 p-2.5 bg-slate-50 hover:bg-slate-100/80 rounded-xl border border-slate-200/60 group/idea transition-all dark:bg-slate-850/50 dark:hover:bg-slate-800/85 dark:border-slate-800"
                    >
                      <p className="text-xs text-slate-600 dark:text-slate-300 font-semibold leading-tight flex-1 break-words">
                        {idea.text}
                      </p>
                      <button
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800 mt-6">
            <div className="flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400 mb-2">
              ...
              <span className="text-indigo-600 dark:text-indigo-400">65%</span>
            </div>
            <div className="w-full h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
              <div className="h-full bg-indigo-600 dark:bg-indigo-500 rounded-full" style={{ width: '65%' }}></div>
            </div>
          </div>
        </aside>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-md shadow-2xl border border-slate-100 dark:border-slate-800 transform transition-all">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-extrabold text-slate-800 dark:text-slate-100 text-xl">Crear Nueva Tarea</h3>
              <button onClick={() => setIsModalOpen(false)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-400 dark:text-slate-500 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Título de la Tarea</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 text-sm font-medium text-slate-800 dark:bg-slate-850 dark:border-slate-700 dark:text-slate-100 dark:focus:ring-indigo-500/40 dark:focus:border-indigo-500"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 text-sm font-medium text-slate-800 resize-none dark:bg-slate-850 dark:border-slate-700 dark:text-slate-100 dark:focus:ring-indigo-500/40 dark:focus:border-indigo-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Fecha y Hora</label>
                  <input
                    type="datetime-local"
                    disabled={noDueDate}
                    required={!noDueDate}
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 text-sm font-medium text-slate-800 disabled:opacity-50 disabled:bg-slate-50 dark:disabled:bg-slate-800/45 dark:bg-slate-850 dark:border-slate-700 dark:text-slate-100 dark:focus:ring-indigo-500/40 dark:focus:border-indigo-500"
                  />
                  <label className="flex items-center gap-2 mt-2 cursor-pointer select-none">
                    <input
                      ...
                      className="rounded border-slate-300 dark:border-slate-700 dark:bg-slate-850 text-indigo-600 focus:ring-indigo-500 w-4 h-4"
                    />
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400">Sin fecha límite</span>
                  </label>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 text-sm font-medium text-slate-800 dark:bg-slate-850 dark:border-slate-700 dark:text-slate-100 dark:focus:ring-indigo-500/40 dark:focus:border-indigo-500"
                  >
                    <option value="" className="dark:bg-slate-900">Selecciona categoría</option>
                    {categories.map((cat) => (
