    ...
    <div className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto bg-slate-100/40 select-none flex flex-col justify-between min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6 md:mb-8">
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 p-1 pr-3 rounded-xl bg-white border border-slate-200">
            ...
            <span className="text-sm font-bold text-slate-700">Alan</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          <div className="flex items-start justify-between">
            ...
            <span className="text-[10px] font-extrabold text-rose-500 uppercase tracking-widest bg-rose-50 px-2.5 py-1 rounded-full">
              Urgente
            </span>
          </div>
          <div className="mt-4">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          <div className="flex items-start justify-between">
            ...
            <span className="text-[10px] font-extrabold text-indigo-500 uppercase tracking-widest bg-indigo-50 px-2.5 py-1 rounded-full">
              Pendiente
            </span>
          </div>
          <div className="mt-4">
            ...
            <div className="flex items-center gap-1 mt-3">
              {pendingTasks.slice(0, 4).map((t, idx) => (
                <div
                  key={t.id}
                  className="w-6 h-6 rounded-full border-2 border-white bg-indigo-500 text-white font-bold text-[9px] flex items-center justify-center -ml-1 first:ml-0"
                  title={t.title}
                >
        ...
        {/* Savings Goal Widget */}
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-between md:col-span-1">
          ...
          {activeSaving ? (
            <div className="flex items-center gap-4">
              ...
              <div className="w-16 h-16 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center flex-shrink-0 relative overflow-hidden">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline">
                  <h4 className="font-bold text-slate-800 text-sm truncate">{activeSaving.title}</h4>
                ...
                <div className="flex justify-between items-center mt-2 text-[10px] font-bold text-slate-700">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-100">
                  <div className="bg-slate-50/50 p-1.5 rounded-lg border border-slate-100 text-center">
                    <p className="text-[8px] font-extrabold text-slate-500 uppercase">Faltan</p>
                    <p className="text-xs font-bold text-indigo-600">${savingNeeded.toLocaleString('en-US')}</p>
                  </div>
                  <div className="bg-slate-50/50 p-1.5 rounded-lg border border-slate-100 text-center">
                    <p className="text-[8px] font-extrabold text-slate-500 uppercase">Proyectado</p>
                    <p className="text-xs font-bold text-slate-700">{getProjectedMonths()}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="text-center py-6">
              <p className="text-xs text-slate-400 font-semibold italic">No tienes metas de ahorro creadas.</p>
              <button
                onClick={() => onViewChange('finanzas')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden flex flex-col justify-between">
            ...
            <div className="divide-y divide-slate-100">
              {noDateTasks.length === 0 ? (
                <div className="px-6 py-12 text-center">
                  ...
                  <p className="text-xs text-slate-300 font-medium mt-1">No hay tareas pendientes sin fecha.</p>
                </div>
              ) : (
                noDateTasks.slice(0, 4).map((task) => (
                  <div
                    key={task.id}
                    className="flex items-start gap-4 px-6 py-4 hover:bg-slate-50/40 transition-colors"
                  >
                    <button
          ...
          <div className="px-6 py-4 border-t border-slate-100 text-center bg-slate-50/20">
            <button
        ...
        <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden flex flex-col justify-between">
          <div>
            ...
            <div className="divide-y divide-slate-100">
              {upcomingEvents.length === 0 ? (
                <div className="px-6 py-12 text-center">
                  <span className="text-3xl mb-2 block">📅</span>
                  <p className="text-sm text-slate-400 font-semibold">Sin eventos próximos</p>
                  <p className="text-xs text-slate-300 font-medium mt-1">Crea tareas con fecha límite para verlas aquí.</p>
                </div>
              ) : (
                upcomingEvents.slice(0, 3).map((event) => {
                  ...
                  return (
                    ...
                    >
                      {/* Date block */}
                      <div className="w-12 h-14 bg-indigo-50 border border-indigo-100 rounded-xl flex flex-col items-center justify-center flex-shrink-0">
                        <span className="text-[9px] font-extrabold text-indigo-500 uppercase">
                          {dateInfo.month}
                        </span>
                        <span className="text-lg font-extrabold text-indigo-700 leading-none mt-0.5">
                          {dateInfo.day}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
          ...
          <div className="px-6 py-4 border-t border-slate-100 text-center bg-slate-50/20">
            <button
              ...
              className="text-xs font-bold text-indigo-600 hover:text-indigo-700 flex items-center gap-1.5 mx-auto justify-center"
            >
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button
            onClick={() => onViewChange('tareas')}
            className="flex-1 md:flex-none px-5 py-3 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold text-xs transition-colors"
          >
            Generar Reporte
          </button>
          <button
